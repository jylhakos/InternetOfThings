...
def create_modern_optimizer(model, optimizer_type="adamw", learning_rate=2e-5, device_type="cuda"):
    """
    Create modern optimizer for BERT fine-tuning with 2025 best practices
    
    Args:
        model: BERT model
        optimizer_type: One of ["adamw", "adafactor", "radam", "adamax"]
        learning_rate: Learning rate
        device_type: "cuda" or "cpu"
        
    Returns:
        optimizer: Configured optimizer
        optimizer_info: Information about the chosen optimizer
    """
    
    print(f"🚀 Creating {optimizer_type.upper()} optimizer for {device_type.upper()}")
    
    if optimizer_type.lower() == "adamw":
        # Default choice - most stable and tested
        optimizer = AdamW(
            model.parameters(),
            lr=learning_rate,
            eps=1e-8,
        ...
        info = "✅ AdamW: Most stable, well-tested optimizer for BERT"
        ...
        info = "💾 Adafactor: Memory efficient, good for large models"
        
    elif optimizer_type.lower() == "radam":
        # Self-correcting Adam - no warmup needed
        optimizer = RAdam(
            model.parameters(),
            lr=learning_rate,
            eps=1e-8,
        ...
        info = "🔬 RAdam: Self-correcting, no manual warmup needed"
        
    elif optimizer_type.lower() == "adamax":
        # Good for sparse gradients
        lr_adjusted = learning_rate * 10  # Adamax typically needs higher LR
        optimizer = Adamax(
            model.parameters(),
            lr=lr_adjusted,
            eps=1e-8,
        ...
        info = f"⚡ Adamax: Good for sparse gradients (LR adjusted to {lr_adjusted})"
        
    else:
        raise ValueError(f"Unknown optimizer: {optimizer_type}. Choose from: adamw, adafactor, radam, adamax")
    
    print(f"   {info}")
    print(f"   Learning rate: {learning_rate}")
    print(f"   Weight decay: 0.01" if optimizer_type != "adafactor" else "   Weight decay: 0.0 (built into Adafactor)")
    
    return optimizer, info

# Configuration for modern optimizer choice
OPTIMIZER_TYPE = "adamw"  # Change this to: "adamw", "adafactor", "radam", or "adamax"

# Create optimizer with modern best practices
optimizer, optimizer_info = create_modern_optimizer(
    model=model,
