...
# Additional utility functions for Dagster integration

def create_dummy_dataset(size=1000):
    """
    Create a dummy dataset for BERT fine-tuning.
    Returns texts and labels for binary sentiment classification.
    """
    import random
    ...
    positive_texts = [
        ...
        "Brilliant service and amazing product!"
    ]
    ...
    negative_texts = [
        ...
        "Cheaply made, broke after one use.",
        "Horrible experience, avoid this seller.",
        "Overpriced for what you get.",
        "Completely defective, doesn't work.",
...
def save_model_artifacts(model, tokenizer, save_directory, training_stats=None):
    """
    Save model artifacts including model, tokenizer, and training statistics.
    """
    import json
    ...
    # Save training statistics if provided
    if training_stats:
    ...
    # Save model info
    model_info = {
        "model_name": model.config.name_or_path if hasattr(model.config, 'name_or_path') else "bert-base-uncased",
...
def train_bert_model(texts, labels, model_name="bert-base-uncased", num_labels=2, 
                    max_length=128, batch_size=16, learning_rate=2e-5, epochs=3):
    """
    Train BERT model with given parameters.
    Returns trained model, tokenizer, and training statistics.
    """
    ...
    # Create datasets
    train_dataset = BertClassificationDataset(train_texts, train_labels, tokenizer, max_length)
    val_dataset = BertClassificationDataset(val_texts, val_labels, tokenizer, max_length)
    ...
    for epoch in range(epochs):
        ...
        training_stats['validation_loss'].append(avg_val_loss)
...
def evaluate_model(model, tokenizer, texts, labels, batch_size=16):
    ...
    eval_dataset = BertClassificationDataset(texts, labels, tokenizer, max_length=128)
